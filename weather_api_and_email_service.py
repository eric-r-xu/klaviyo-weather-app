# Eric Xu, 2019-09
# weather_api_service & weather_email_service
# runs on cron daily at 9 am
...
def weather_api_service(cityIDset, dateFact, tomorrow, mysql_conn, city_dict):
	...
	for _i, cityID in enumerate(cityIDset):
		...
		r = requests.get('http://api.openweathermap.org/data/2.5/weather?id=%s&appid=%s' % \
			(cityID, OPENWEATHERMAP_AUTH['api_key']))
		obj = r.json()
		today_weather = '-'.join([obj['weather'][0]['main'], obj['weather'][0]['description']])
		today_max_degrees_F = K_to_F(obj['main']['temp_max'])
		time.sleep(1.5) # reduce cadence of api calls 
		# forecast weather api call
		r = requests.get('http://api.openweathermap.org/data/2.5/forecast?id=%s&appid=%s' % \
			(cityID, OPENWEATHERMAP_AUTH['api_key']))
		obj = r.json()
		...
		query = "INSERT INTO klaviyo.tblFactCityWeather(city_id, dateFact, today_weather, today_max_degrees_F, \
		tomorrow_max_degrees_F) VALUES (%i, '%s', '%s', %i, %i)" % (cityID, dateFact, today_weather, today_max_degrees_F, \
			tomorrow_max_degrees_F)
		runQuery(mysql_conn, query)
		logging.info('%s - %s - %s - %s - %s - %s' % (city_dict[str(cityID)], cityID, dateFact, today_weather, \
							      today_max_degrees_F, tomorrow_max_degrees_F))
		if _i % 20 == 5:
			gc.collect();
...
def weather_email_service(email_service, app, mysql_conn, city_dict):
	...
	tfcw_df = pd.read_sql_query('SELECT city_id, today_weather, today_max_degrees_F, tomorrow_max_degrees_F \
		FROM klaviyo.tblFactCityWeather where dateFact=CURRENT_DATE and city_id in (%s)' % (city_id_string), con=mysql_conn)
	tblFactCityWeather_dict = dict()
	zipped_array = zip(tfcw_df['city_id'], tfcw_df['today_weather'], tfcw_df['today_max_degrees_F'], tfcw_df['tomorrow_max_degrees_F'])
	for city_id, today_weather, today_F, tomorrow_F in zipped_array:
    		tblFactCityWeather_dict[city_id] = [str(today_weather).lower(), int(today_F), int(tomorrow_F)]
		...
		with app.app_context():
			with email_service.connect() as conn:
				for recipient in recipients:
					msg = Message(subject_value,recipients=[recipient],sender="eric.r.xu@gmail.com")
					msg.html = """ %s - %s degrees F - %s <br><br><img src="%s" \
					width="640" height="480"> """ % (city_dict[str(city_id)], \
									 today_F, today_weather, gif_link)
					try:
						conn.send(msg)
					except:
